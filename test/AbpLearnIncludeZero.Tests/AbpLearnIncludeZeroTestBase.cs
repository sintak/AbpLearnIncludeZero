...
{
    public abstract class AbpLearnIncludeZeroTestBase : AbpIntegratedTestBase<AbpLearnIncludeZeroTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AbpLearnIncludeZeroTestBase()
        {
        ...
        protected void UsingDbContext(Action<AbpLearnIncludeZeroDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpLearnIncludeZeroDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpLearnIncludeZeroDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpLearnIncludeZeroDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpLearnIncludeZeroDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpLearnIncludeZeroDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpLearnIncludeZeroDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpLearnIncludeZeroDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpLearnIncludeZeroDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpLearnIncludeZeroDbContext, Task<T>> func)
        {
            T result;
